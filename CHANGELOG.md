...
- Chrome 扩展新增「选句循环播放」：转写完成后可勾选一句或多句，开启「循环」
  后播到句尾自动跳到下一选中句句首、最后一句结束回到第一句；未选句时开启
  循环默认选中当前播放句；快捷键 Ctrl/^+Shift+L（macOS 用 Control）。
- `POST /transcribe`（type=bv）增加重复检测：同一 BV 已在排队/运行中或已有
  成功结果（结果文件仍在）时直接复用原任务，不再重复下载转写；响应新增
  `skipped` / `reason` / `result_path` 字段，并提供 `force` 参数与 CLI
  `--force` 强制重新解析。
