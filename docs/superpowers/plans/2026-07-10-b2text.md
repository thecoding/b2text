...
### Task 7: diarizer.py — 说话人日志封装（独立 API）

**Files:**
- Create: `b2text/diarizer.py`
- Test: `tests/test_diarizer.py`

**理由**：spec 架构图中将 diarizer.py 列为独立模块。本任务提供"纯说话人日志"（不要文字）的 API 接口，封装 FunASR AutoModel。CLI 默认走 transcriber.py 的合并路径；该独立 API 可用于未来"对已有字幕做说话人归属"等场景。
...
def test_lazy_loads_model_on_first_call():
    ...
    fake_model = MagicMock()
    fake_model.generate.return_value = [
        {"start": 0.0, "end": 1.0, "text": "", "spk": 0},  # 说话人日志不需文字
    ]
...
```bash
git add b2text/diarizer.py tests/test_diarizer.py
git commit -m "feat: add diarizer.py for standalone speaker diarization"
```

---

### Task 8: B站 API 客户端（cid 获取、下载 URL 解析）
...
### Task 9: CLI 入口 bilibili_to_text.py
